...
public class GUISpriteManager : MonoBehaviour 
{
	...
	*/
    ...
    // Returns the index of the first newly allocated element (ex: if the sprite array was already 10 elements long and is 
    // enlarged by 10 elements resulting in a total length of 20,  EnlargeArrays() will return 10, indicating that element 10 is the 
    ...
    protected int EnlargeArrays( int count )
    {
        ...
        for( int i = firstNewElement; i < sprites.Length; ++i )
        {
			...
			sprites[i].vertexIndices.initializeVertsWithIndex( i );
